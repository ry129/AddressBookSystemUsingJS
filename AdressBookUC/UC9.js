...
class AddressBook {
  ...
  searchContacts(Name, value) {
    let data = this.contacts.filter(
      (c) => c.city === value && c.firstName === Name
    );
    if (data.length == 0) {
      console.log("Person not present in the city.");
    } else {
      console.log("Person present in the city.");
      return data;
    }
  }
}
...
console.log(addressBook.searchContacts("Jiya", "Datia"));
